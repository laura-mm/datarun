		...
		for (int j = 0; j < i; j++)
		{
			for (int k = 0; k < j; k++)
			{
				for (int l = 0; l < k; l++)
				{
					...
					//cout << A(i,j,k,l) << ", " << A(j,k,l,i) << ", " << A(k,l,i,j) << ", " << A(l,i,j,k) << endl;
				}
			}
		}
	}
...
struct output_state_and_time
{
	...
	void operator()(const state_type &x, double t)
	{
		...
		//if (coutall && (*times).size() != 0) cout << "tstep " << t - (*times).back() << endl;
...
class simulation
{
	...
	static void rhs( const state_type &x_array , state_type &dxdt , const double  t, Tensor<double, 4> &C, Tensor<double, 1> k)//, double tcheck)
	{
		auto x(TensorMap<Tensor<const double, 1>>(x_array.data(), N));
		Tensor<double, 1> dxdt_tensor = x * (k - x + C.contract(x, d10).contract(x, d10).contract(x, d10)); // change for different orders
		dxdt = Map<Array<double,N,1>>(dxdt_tensor.data());
		if (coutall) coutfile << "max dxdt " << dxdt.maxCoeff() << endl;
		if (dxdt.maxCoeff() > 100.0/* && t < 20.0*/) throw string("diverged");
	}
	...
	void run(double start, double end)
	{
		if (ode_method == "rk4_fixed")
		{
			...
			{	
				...
				if (coutall) coutfile << t << ", min " << xstate.minCoeff() << ", max " << xstate.maxCoeff() << endl;
		...
		else if (ode_method == "rk4fixed_boost")
		{
			...
			integrate_const(stepper, bind(rhs, _1, _2, _3, C, k/*, (*times).back()*/), xstate, start, end, dt, output_state_and_time(trajtfile, traj, times, stat, &stime)); // change
		}
		else if (ode_method == "adaptive_boost")
		{
			// Default adaptive method
			//state_type xInit = Tarr(x,N);
			integrate(bind(rhs, _1, _2, _3, C, k/*, (*times).back()*/), xstate, start, end, dt, output_state_and_time(trajtfile, traj, times, stat, &stime)); // change
		}
		else if (ode_method == "adaptive_boost_error_controlled")
		{
			...
			size_t steps = integrate_adaptive(
				...
				bind(rhs, _1, _2, _3, C, k/*, (*times).back()*/), xstate, start, end, dt, // change
				output_state_and_time(trajtfile, traj, times, stat, &stime));
		}
	//static void rhs( const state_type &x_array , state_type &dxdt , const double /* t */, Tensor<double, 4> &C, Tensor<double, 1> k, double tcheck) for reference
		else
		{
...
void datarun()
{
	...
	coutfile.open("cout" + to_string(p) + "_" + to_string(v1) + "_" + to_string(v2) + "_" + to_string(v3) + "_" + to_string(r) + ".txt");
...
int main(int argc, char** argv) // p = 4
{
	...
	//cout << "sigma " << sigma(p-2) << ", mu " << mu(p-2) << ", gamma " << gama(p-2) << endl;
