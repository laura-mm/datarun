...
for gii in range(4):
	...
	z = np.loadtxt("bunin4_{0}.txt".format(gi), delimiter = ",")
	z = z.reshape(len(z)//2, 2)
	z = np.transpose(z)
	plt.semilogy(z[0], z[1], col[gi], linestyle = ':')
	
	m = np.loadtxt("mu4_{0}.txt".format(gi), delimiter = ",")
	m = m.reshape(len(m)//3, 3)
	m = np.transpose(m)
	plt.semilogy(m[0], m[1], col_[gi])
	plt.semilogy(m[0], m[2], col[gi], linestyle = 'dashdot')
	
	plt.plot(0, 0, col[gi], label = gammalab4[gi])
	
	plt.ylim([10**(-2), 10**2])
	plt.xlim([-4, 4.5])
	plt.xlabel("$\mu_4$", fontsize = 15)
	plt.ylabel("$\sigma_4$", fontsize = 15)
...
colour = np.loadtxt("allcolour4c50.txt", delimiter = ",")
...
for gi in range(6):
	...
	plt.semilogy(m[0], m[2], col[gi], linestyle = 'dashdot', linewidth = 3)
