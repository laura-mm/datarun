...
class simulation
{
	...
	static void rhs( const state_type &x_array , state_type &dxdt , const double /* t */, Tensor<double, 3> &B, Tensor<double, 1> k)
	{
		auto x(TensorMap<Tensor<const double, 1>>(x_array.data(), N));
		Tensor<double, 1> dxdt_tensor = x * (k - x + B.contract(x, d10).contract(x, d10)); // change for different orders
		dxdt = Map<Array<double,N,1>>(dxdt_tensor.data());
		if (coutall) coutfile << "max dxdt " << dxdt.maxCoeff() << endl;
		//if (dxdt.maxCoeff() > 50.0) throw string("diverged");
	}
	...
	void run(double start, double end)
	{
		if (ode_method == "rk4fixed_boost")
		{
			...
			integrate_const(stepper, bind(rhs, _1, _2, _3, B, k), xstate, start, end, dt, output_state_and_time(trajtfile, traj, times, stat, &stime)); // change
		}
		else if (ode_method == "adaptive_boost")
		{
			// Default adaptive method
			//state_type xInit = Tarr(x,N);
			integrate(bind(rhs, _1, _2, _3, B, k), xstate, start, end, dt, output_state_and_time(trajtfile, traj, times, stat, &stime)); // change
		}
		else if (ode_method == "adaptive_boost_error_controlled")
		{
			...
			size_t steps = integrate_adaptive(
				...
				bind(rhs, _1, _2, _3, B, k), xstate, start, end, dt, // change
