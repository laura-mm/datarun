...
points = [] # gi, 012, mi
for gi in range(6):
	p = np.loadtxt("points4_{0}.txt".format(gi), delimiter = ",")
	p = p.reshape(25, 3)
	p = np.transpose(p)
	points.append(p)
	

meas = np.loadtxt("allfive4.txt", delimiter=",")
...
for mi in range(25):
	measfp = []
	for gi in range(6):
		g = np.loadtxt("measfp4_{0}_{1}.txt".format(mi, gi), delimiter = ",")
...
for gi in range(6):
	...
	if (gi == 3 or gi == 4 or gi == 5):
		plt.xlabel("$\sigma_4$", fontsize = 15)
	if (gi == 0 or gi == 3):
...
for mi in range (4):
	...
	plt.xlim([sigmaval[0], sigmaval[-1]])
	plt.title("$\mu_4 = {0}$".format(muval[mival[mi]]), fontsize = 15)
	plt.ylim([0, Mlim[mi]])
	plt.xlabel("$\sigma_4$", fontsize = 15)
	if (mi == 0):
		plt.ylabel("$M^*$", fontsize = 15)
